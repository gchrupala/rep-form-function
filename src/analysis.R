...
omi_v  <- omi %>% select(-omission_t, -omission_lm, -omission_sum) %>% rename(score=omission_v) %>% mutate(pathway="visual")
omi_t  <- omi %>% select(-omission_v, -omission_lm, -omission_sum) %>% rename(score=omission_t) %>% mutate(pathway="textual")
omi_lm <- omi %>% select(-omission_t, -omission_v, -omission_sum)  %>% rename(score=omission_lm) %>% mutate(pathway="lm")
omi_sum <- omi %>% select(-omission_v, -omission_t, -omission_lm)  %>% rename(score=omission_sum) %>% mutate(pathway="sum")
...
data <- rbind(data_t, data_v) %>% rbind(data_lm) %>% rbind(data_sum) %>%
...
# figure fig:omissionex
labels <- (data %>% filter(sentid==69649, pathway=='visual') %>% arrange(position))$word
ggplot(data %>% filter(sentid==69649, pathway!="sum"), aes(x=position, y=score, color=pathway)) +
  geom_line() +
  scale_x_continuous(breaks=0:(length(labels)-1), labels=labels) +
  xlab("") +
  theme(legend.title=element_blank()) +
  theme(aspect.ratio=2/3, text=element_text(size=25)) +
  theme(axis.text.x=element_text(angle=45, hjust=1))
ggsave("../doc/omissionex.pdf", width=12, height=10)





# FIGURE fig:omission-imaginet
ggplot(data %>% filter(pathway != "sum") %>% filter_by_count("postag", MINCOUNT)) +
...
ggplot(data %>% filter(pathway != "sum") %>% filter_by_count("dep", MINCOUNT)) +
...
# FIGURE fig:omission-imaginet-ratio

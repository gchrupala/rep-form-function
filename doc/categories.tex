...
\begin{figure*}[t]
  ...
  \caption{Omission scores for an example sentence for {\sc LM} and
...
measure for the {\sc LM}, {\sc Visual} and {\sc Textual} networks for an
example captions. The $\mathrm{omission}$ scores are
plotted for the
...
Figure~\ref{fig:omissionexpic} shows the images retrieved by {\sc
  Visual} for the full
caption and for the one with the word {\sc baby} omitted. 
...
Bi-directional RNNs, are sensitive by word-order and can potentially
...
grammatical function in sentences, {\it even after controling for word
...
increase in $R^2$ between the {\sc LR~word} and {\sc LR~dep} predictors
...
explains more of the variance in case of {\sc Visual} and especially
